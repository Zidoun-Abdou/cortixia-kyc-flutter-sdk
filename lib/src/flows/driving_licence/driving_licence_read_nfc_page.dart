...
class _DrivingLicenceReadNfcPageState extends State<DrivingLicenceReadNfcPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _saveFaceImage() async {
    // IDL applet stores the portrait under the 'dg5' key (SFI 0x04, tag
    // 0x65). Fall back to 'dg2' for the legacy ID-card shape.
    final dg = (_decoded?['dg5'] ?? _decoded?['dg2']) as Map<String, dynamic>?;
    if (dg != null && dg['result'] == 'True') {
      final faceB64 = dg['face'] as String?;
      if (faceB64 != null && faceB64.isNotEmpty) {
        try {
          final bytes = base64Decode(faceB64);
          final directory = await getTemporaryDirectory();
          final filePath = path.join(directory.path, 'face_from_drivinglicence.jpg');
