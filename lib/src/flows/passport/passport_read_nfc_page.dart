...
class PassportReadNfcPage extends StatefulWidget {
  final List<CameraDescription> cameras;
  final String passportNumber;
  final String dob;
  final String doe;

  const PassportReadNfcPage({
    super.key,
  ...
  @override
  State<PassportReadNfcPage> createState() => _PassportReadNfcPageState();
}

class _PassportReadNfcPageState extends State<PassportReadNfcPage> with SingleTickerProviderStateMixin {
  final _log = Logger('cortixiakyc.passport.nfc');
  final _nfc = NfcProvider();
  bool _reading = false;
  String _status = '';
  _PassportMrtdData? _data;
  ...
  Future<void> _readMRTD() async {
    setState(() {
      _reading = true;
      _status = '📱 Approchez le passeport du téléphone…';
      _data = null;
      _decoded = null;
    });
    ...
    try {
      ...
      final bac = DBAKeys(widget.passportNumber, dob, doe);

      await passport.startSession(bac);

      final out = _PassportMrtdData();
  ...
  Future<void> _decodeLocally() async {
    ...
    try {
      ...
      // The server decodes the datagroups and returns the passport's
      // JPEG-2000 portrait/signature already converted to JPEG.
      ...
      final decision = await host.documentRead(
        data,
        mrzDocumentNumber: widget.passportNumber,
    ...
    if (dg2 != null && dg2['result'] == 'True') {
      ...
      if (faceB64 != null && faceB64.isNotEmpty) {
        try {
          ...
          final filePath = path.join(directory.path, 'passport_face.jpg');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Lecture NFC - Passeport'),
            ...
            children: [
              if (!(_data != null && _decoded != null)) ...[
                ...
                Container(
                  ...
                  child: Text(
                    _status.isEmpty ? 'Prêt à scanner le passeport' : _status,
                    style: TextStyle(
                      fontSize: 14,
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: const [
                                  Icon(Icons.storage, color: Color(0xFFD97706)),
                                  SizedBox(width: 12),
                                  Text(
                                    'Données brutes lues',
                                    style: TextStyle(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: const [
                  Icon(Icons.person, color: Color(0xFFD97706)),
                  SizedBox(width: 12),
                  Text(
              ...
              const SizedBox(height: 12),
              _infoRow('👤 Prénom', dg11['name_latin'] ?? ''),
              _infoRow('👥 Nom', dg11['surname_latin'] ?? ''),
              _infoRow('🎂 Date de naissance', dg11['birth_date'] ?? ''),
              _infoRow('📍 Lieu de naissance', dg11['birthplace_latin'] ?? ''),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _infoRow('🏘 Commune', dg12['commune_latin'] ?? ''),
              _infoRow('📅 Date de création', dg12['creation_date'] ?? ''),
            ],
          ),
        ),
      ));
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: const [
                  Icon(Icons.photo_camera, color: Color(0xFFD97706)),
                  SizedBox(width: 12),
                  Text(
                    'Photo d\'identité',
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF111827)),
